...
from .formatting import elapsed_pct, field_period, format_credits, format_tooltip, parse_field_name, popup_label, time_until
from .i18n import T
from .popup import UsagePopup
from .splash import SplashScreen
from .auto_update import check_for_update, latest_version, latest_url
...
class UsageMonitorForClaude:
    ...
    def __init__(self) -> None:
        ...
        self._active_popup = None  # reference to open UsagePopup instance
        self._popup_close_event = threading.Event()
        ...
        self.icon = pystray.Icon(
            ...
            menu=pystray.Menu(
                ...
                pystray.Menu.SEPARATOR,
                pystray.MenuItem('About ClaudeMeter', self.on_about),
                pystray.MenuItem(
                    lambda item: f'Update available: v{latest_version}' if latest_version else 'Check for updates',
                    self.on_open_update,
                    visible=lambda item: True,
                    enabled=lambda item: bool(latest_version),
                ),
                pystray.Menu.SEPARATOR,
    ...
    def on_show_popup(self, icon: Any = None, item: Any = None) -> None:
        with self._popup_lock:
            if self._popup_open:
                # Already open — signal it to close via event
                self._popup_close_event.set()
                return
            if time.time() - self._popup_closed_at < 0.15:
                return
            self._popup_open = True
            self._popup_close_event.clear()
        threading.Thread(target=self._open_popup, daemon=True).start()

    def on_open_update(self, icon: Any = None, item: Any = None) -> None:
        """Open the releases page in the browser."""
        import webbrowser
        from .auto_update import latest_url
        webbrowser.open(latest_url)

    def on_about(self, icon: Any = None, item: Any = None) -> None:
        """Show About dialog with version and attribution info."""
        from . import __version__, __base_version__, __base_author__, __base_url__
        msg = (
            f'ClaudeMeter  v{__version__}\n'
            f'Claude API Usage Monitor\n'
            f'Developed by Wayne O\n'
            f'\n'
            f'Built on usage-monitor-for-claude v{__base_version__}\n'
            f'by {__base_author__}\n'
            f'{__base_url__}\n'
            f'\n'
            f'Licensed under MIT License'
        )
        ctypes.windll.user32.MessageBoxW(0, msg, 'About ClaudeMeter', 0x40)
    ...
    def _open_popup(self) -> None:
        # Store reference so hotkey toggle can close it
    ...
    def _render_tray(self) -> None:
        ...
        # Pystray bug: _update_title is never called automatically
        # so we call it directly to force the Win32 tooltip update
        try:
            self.icon._update_title()
        except Exception:
            pass

    def _build_tooltip(self, data: dict) -> str:
        """Build tray tooltip with usage percentages and reset countdowns."""
        if 'error' in data:
            return 'ClaudeMeter — error fetching usage'
        ...
        for field in ['five_hour', 'seven_day']:
            ...
            # Use compact reset format to stay within 128 char tooltip limit
            reset_str = time_until(resets_at) if resets_at else ''
            # Strip the clock time "(13:00)" from reset string to save chars
            if reset_str and '(' in reset_str:
                reset_str = reset_str[:reset_str.rfind('(')].strip()
            line = f'{label}: {pct:.0f}%'
            if reset_str:
                line += f' ({reset_str})'
            lines.append(line)
        ...
        if extra.get('is_enabled') and extra.get('monthly_limit'):
            ...
            if limit > 0:
                pct = used / limit * 100
                lines.append(f'Extra: {pct:.0f}%')

        lines.append('Ctrl+Shift+C toggle')
        return '\n'.join(lines)[:127]
    ...
    def _on_theme_changed(self) -> None:
        ...
        # Notify popup to update its colour scheme if open
        try:
            popup = getattr(self, '_active_popup', None)
            if popup and popup._shown:
                theme = 'light' if light else 'dark'
                popup._window.evaluate_js(f'applySystemTheme("{theme}")')
        except Exception:
            pass
    ...
    def update(self) -> None:
        ...
        # Write history snapshot (30-min slot throttled)
        try:
            from .popup import _write_history, _build_usage_for_history, _write_daily_summary
            snap = self.cache.snapshot
            if snap.usage:
                threading.Thread(
                    target=_write_history,
                    args=(_build_usage_for_history(snap.usage),),
                    daemon=True
                ).start()
            # Check if daily summary needs writing (yesterday not yet summarised)
            threading.Thread(target=_write_daily_summary, daemon=True).start()
        except Exception:
            pass
        ...
        for key, pct in quota_fields.items():
            ...
            if prev > reset_threshold and pct < prev and not any_blocking:
                from .popup import _load_prefs as _lp
                if _lp().get('notify_on_reset', False):
    ...
    def _get_user_thresholds(self, variant_key: str) -> list[float]:
        """Get alert thresholds respecting user prefs from settings panel."""
        from .popup import _load_prefs
        prefs = _load_prefs()
        key = variant_key.lower()
        # Session fields contain 'hour' or 'five'
        if 'hour' in key or key.startswith('five'):
            pct = prefs.get('notify_session_pct', 80)
            return [float(pct)]
        # Weekly fields contain 'day' or 'week' or 'seven'
        if 'day' in key or 'week' in key or key.startswith('seven'):
            pct = prefs.get('notify_weekly_pct', 80)
            return [float(pct)]
        return get_alert_thresholds(variant_key)
    ...
    def _check_threshold_alerts(self, data: dict[str, Any]) -> None:
        ...
        for variant_key, entry in data.items():
            ...
            thresholds = self._get_user_thresholds(variant_key)
            if not thresholds:
                continue
            ...
            # Skip time-aware suppression when user has set a custom threshold
            # (custom thresholds should always fire regardless of time position)
            from .popup import _load_prefs as _lp
            _p = _lp()
            using_custom = 'notify_session_pct' in _p or 'notify_weekly_pct' in _p
            if ALERT_TIME_AWARE and not using_custom and highest_exceeded > last_notified and highest_exceeded < ALERT_TIME_AWARE_BELOW:
    ...
    def _on_icon_ready(self, icon: Any) -> None:
        ...
        try:
            ...
            # Update splash progress and close once ready
            ...
            threading.Thread(target=start_hotkey_listener, args=(self.on_show_popup,), daemon=True).start()
    ...
    def run(self) -> None:
        # Show splash screen while initialising
