...
# Claude brand orange — used for the 'C' glyph when usage is zero (idle state)
_CLAUDE_ORANGE = (217, 119, 87, 255)
...
def create_icon_image(
    ...
    *, mode_top: str = 'utilization', mode_bottom: str = 'utilization',
    ...
    tw = bbox[2] - bbox[0]
    # Use Claude orange for the idle 'C' glyph; percentage/symbols use normal fg
    glyph_color = _CLAUDE_ORANGE if text == 'C' else fg
    draw.text(((S - tw) / 2 - bbox[0], -bbox[1]), text, fill=glyph_color, font=font,
              stroke_width=stroke_width, stroke_fill=glyph_color)
    ...
    # Bar colours: top=session (red when warn, blue normal), bottom=weekly (blue)
    _BAR_COLORS = [
        ((224, 80, 80, 255),   (74, 158, 255, 255)),   # top: warn=red, normal=blue
        ((224, 80, 80, 255),   (74, 158, 255, 255)),   # bottom: same scheme
    ]
    for idx, (y, pct, mode, time_pct) in enumerate((
        (bar1_y, pct_top, mode_top, time_pct_top),
        (bar2_y, pct_bottom, mode_bottom, time_pct_bottom),
    )):
        warn_pct = time_pct if time_pct is not None else 80
        bar_warn = pct >= 80 or (time_pct is not None and pct > time_pct)
        bar_color = _BAR_COLORS[idx][0] if bar_warn else _BAR_COLORS[idx][1]
        ...
        if mode == 'overage' and time_pct is not None and time_pct < 100:
            ...
            if fill_w > 0:
                draw.rectangle([0, y, fill_w - 1, y + bar_h - 1], fill=bar_color)
        else:
